...
const table = async (name, options = {}) => {
  try {
    ...
    let sequences = await db.any('SELECT sequence_name FROM information_schema.sequences WHERE sequence_name like $1', [`${name}_%_seq`]);
    ...
    columns = columns.map(column => {
      const sequence = sequences.find(x => x == column.name); 
      const sequence2 = sequences.find(x => column.primaryKey && column.name.includes(x));
    ...
    if (!options.limit && !options.offset) {
      ...
      query = `SELECT * FROM ${name} LIMIT ${options.limit} OFFSET ${options.offset}`;
    }
