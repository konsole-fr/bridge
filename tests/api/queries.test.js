...
describe('POST /queries', () => {
  describe('when query is missing', () => {
    test('returns 400', async () => {
      const res = await request(api).post('/api/queries');
  ...
  describe('when query is invalid', () => {
    test('returns 500', async () => {
      const res = await request(api).post('/api/queries').send({ sql: 'azerty' });
  ...
  describe('when query is valid', () => {
    test('returns the query results', async () => {
      const res = await request(api).post('/api/queries').send({ sql: 'select 1+1 as sum' });
